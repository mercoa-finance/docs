...
description: Use Mercoa to orchestrate and trigger your custom payment rails
---
...
Custom payment rails are designed to be used along with our [webhooks](/webhooks) functionality.
...
[API Reference](/api-reference/payment-method/custom-schema)
...
You can define this schema using the [POST /paymentMethod/schema](/api-reference/payment-method/custom-schema/create) endpoint:
...
The custom `paymentMethod` will contain the data that is required to trigger a payment using your custom payment rail, and a `foriegnId` that can be used to identify the payment method in your system.
<Note>
...
For example, once you have defined a custom payment rail schema for a wire transfer, you can create a `paymentMethod` that uses that schema using the [POST /paymentMethod](/api-reference/payment-method/create) endpoint:
